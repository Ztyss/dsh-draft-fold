...
`;
		...
		/** Loader entry id — on 0.1.7+ it doubles as the settings namespace (configForms key). */
		const ENTRY_ID = "@ztyss/dsh-draft-fold";
		...
		/** Effective limits before any settings face attaches: the plain defaults. */
		const DEFAULT_EFFECTIVE = effectiveFromSnapshot(void 0);
		...
		function createDeferredEffectiveStore() {
			let live = void 0;
			let offLive = void 0;
			const listeners = new Set();
			const emit = () => {
			...
			return {
				getSnapshot: () => live !== void 0 ? live.getSnapshot() : DEFAULT_EFFECTIVE,
				subscribe(listener) {
					listeners.add(listener);
					return () => listeners.delete(listener);
				},
				/** Bind onto the runtime settings face once its service activates (legacy scope or 0.1.7+ form). */
				attach(scope) {
					offLive?.();
					live = createEffectiveStore(scope);
					offLive = live.subscribe(emit);
					emit();
				}
			};
		}
			...
			return (0, react_jsx_runtime.jsxs)("li", {
				...
				children: [(0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						/* 自有 chevron 字形：内核 primitives 的尺寸命名图标（IconChevronDownOutline14）
						   在 0.1.7 改名为笔画命名（…Medium/Regular），内联 SVG 与内核图标体系解耦。 */
						(0, react_jsx_runtime.jsx)("svg", {
							className: "dff-chevron" + (open ? " dff-chevronOpen" : ""),
							viewBox: "0 0 14 14",
							"aria-hidden": true,
		...
		//#endregion
		//#region src/client/DraftFoldTab.tsx
		/** Plugins-tab page shell (0.1.7+): the settings card inside a plain list wrapper. */
		function DraftFoldTab(props) {
			return (0, react_jsx_runtime.jsx)("ul", {
				className: "dff-tabCards",
				children: (0, react_jsx_runtime.jsx)(DraftFoldCard, props)
			});
		}
		//#endregion
		//#region src/client/index.tsx
		/** Required runtime services for the browser half. Both kernels provide these. */
		...
		function apply(ctx) {
			...
			// 内核 ≤0.1.5：settingsScope 服务。绑定 scope、接通 dock store，并在注入回调里
			// 延迟注册 插件→插件配置 卡（keyed 槽位身份是 key，字段只留 name/key/locale/inject）。
			// 0.1.7+ 已删除该服务，这条注入永不触发（dry-run 实证：不崩、不阻塞 boot）。
			ctx.inject(["settingsScope"], (scoped) => {
				try {
					store.attach(scoped.settingsScope.bind({ namespace: SETTINGS_NS }));
					...
					console.error("[dsh-draft-fold] legacy settings wiring failed", error);
				}
			});
			// 内核 0.1.7+：settings 改由 loader-entry Config 投影（namespace == entry id），
			// configForms 服务按 entry id 暴露表单；插件配置页渲染注册进
			// settings.plugins.tab 的页面。服务缺失（≤0.1.5）时同样永不触发。
			ctx.inject(["configForms"], (formCtx) => {
				try {
					const form = formCtx.configForms.get(ENTRY_ID);
					store.attach(form);
					const card = new DraftFoldCardController(form);
					const tabT = formCtx.locale.bind(NS);
					formCtx.slots.inject("settings.plugins.tab", () => formCtx.slots.register({
						name: "settings.plugins.tab",
						id: SETTINGS_NS,
						order: 90,
						locale: NS,
						label: () => tabT("settings.title"),
						inject: () => card.inject()
					}, DraftFoldTab));
				} catch (error) {
					console.error("[dsh-draft-fold] settings tab register failed", error);
				}
			});
		}
		//#endregion
		...
		exports.DraftFoldTab = DraftFoldTab;
		exports.createDeferredEffectiveStore = createDeferredEffectiveStore;
